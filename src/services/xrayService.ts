...
import type { XRayAlert, XRayReport } from "@/types/app/analytics";
import type { FundTopHolding } from "@/types/app/quote";

/**
 * 历史遗留兼容：旧 alerts 里可能含"XX 行业暴露过高"这条（我们已经把行业维度整体拿掉了，
 * 但没重新扫描的用户报告里仍旧存在），读取时直接剥掉，不再上屏。
 */
function filterLegacyAlerts(alerts: XRayAlert[]): XRayAlert[] {
  return alerts.filter((a) => !/行业/.test(a.title) && !/行业/.test(a.message));
}

function toReport(row: Record<string, unknown>): XRayReport {
  const rawAlerts = (row.alerts ?? []) as XRayAlert[];
  return {
    id: String(row.id),
    ...
    alerts: filterLegacyAlerts(rawAlerts),
 ...
 * 这些基金回退到静态底稿，底稿也没收录的直接列入 unmatched_funds，
 * 不会静默丢失。
